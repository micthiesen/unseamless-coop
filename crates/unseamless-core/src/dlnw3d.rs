...
//! transport is not network-byte-order). The token semantics (`member+0x148`, an echoed value or session
//! nonce) remain opaque, so `frame_type5` takes it as a parameter. Keep this module host-tested; the cdylib
//! hands this plaintext message to the live `FsdpConnection`, which owns route and transport encoding.
