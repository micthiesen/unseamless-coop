...
| `rung3-create-drive` | The **two-machine rung-3 create-drive run** (`docs/RUNG3-DRIVE-RUNBOOK.md`): connect (roles derived by the connect step, link auto-finishes on `coop: linked`), then WATCH the one-shot driven create — the watch step auto-finishes on the driver's `drive-create returned` line (or the live lobby FSM) and **branches on the captured verdict**: pass / cap-zero / partial / inspect, converging on an Export step. Ends with a committed **stub** (the join-side driver doesn't exist yet). Run with `[debug.probes] session_probe + drive_create + force_netsession_ready` and `[gameplay] bypass_session_create_gate` on **both** machines — the full staging is the runbook. |
