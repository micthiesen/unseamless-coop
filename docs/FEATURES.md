...
| Load mechanism | `ersc.dll` exports `modengine_ext_init` → it's a **ModEngine2 extension**; also ships `ersc_launcher.exe` (exe-swap). | **M** | Our harness already loads via Elden Mod Loader; ModEngine2 ext API is publicly documented. |
...
| Offline / non-EAC launch | Runs outside EasyAntiCheat (why it's co-op-safe). | **M** | Launch concern, partly handled by the loader path. |
