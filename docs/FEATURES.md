...
|---|---|---|---|
| Session / networking layer | Replaces vanilla matchmaking so partners share one persistent world. Confirmed deps: `steam_api64` (Steam P2P), `ws2_32` (Winsock), `crypt32`/`wldap32`/`normaliz` (TLS/crypto). | **H; two-machine roster proven** | The 80%. **Legally most sensitive** (wire protocol), so behavior is reimplemented from clean-room observations. The out-of-band stack (rungs 1/2/4) is shipped and two-machine-proven. Rung 3 now drives the game's native session graph far enough that both rig and Deck hold `Host`/`Ingame`, exchange Steam-authenticated native DLNR3D traffic, and report the remote peer in `players=2` (2026-07-13). Remaining gates are the visual/control check that the peer is present in-world and productizing the proven debug path behind real Open/Join peer discovery. See [SESSION-DRIVE.md](SESSION-DRIVE.md) > "Native Transport and Two-Player Roster Result (2026-07-13)". |
...
| Stay connected ([SESSION-LIFECYCLE-FINDINGS.md](SESSION-LIFECYCLE-FINDINGS.md)) | Suppress every *game-driven* co-op disconnect (boss defeat, area transition, death, host-grace timeout, remote leave packet) so the session survives the events vanilla ends it on — the other core "seamless" behavior, alongside roaming. | **built; rig-gated (default off)** | `coop/stay_connected.rs` hooks the two charted leave sites (`leave_session 0x140cae730` + the update-task inline twin `0x140cb0840`) behind a runtime-armed flag held to `gameplay.stay_connected` (menu-togglable live; hooks install only when on at boot). Byte-verified install, degrades to vanilla on drift or when the `session_probe` leave tracer owns the site. Suppressions aggregate through the host-tested `core::stay_connected::SuppressAnnouncer` into a milestone log + ER-voiced toast. A genuine peer drop still tears down (the async transport handler `0x1423f46d0` never runs the gated sites — by design). **Rig pass pending:** findings-doc risks #1–#3 (no third `lobby_state=7` writer; boss/area/death actually route here; suppressed-leave sibling effects stay playable). Future work (out of scope here): the additive re-sync layer across area transitions. |
...
4. **M3 — minimal shared session:** two players, position + basic state sync, one shared world.
   "First playable." <- current frontier. The native session roster is two-machine-proven at `players=2`; visual/control presence and Open/Join productization remain.
